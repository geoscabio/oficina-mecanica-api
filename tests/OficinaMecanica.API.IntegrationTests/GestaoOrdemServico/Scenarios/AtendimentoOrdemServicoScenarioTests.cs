...
public sealed class AtendimentoOrdemServicoScenarioTests : ApiIntegrationTestBase
{
    ...
    public async Task Dado_DadosValidos_Quando_ExecutarAtendimentoCompleto_Entao_DeveEntregarOrdemServicoECalcularTempoMedio()
    {
        ...
        var ordemServico = await PostJsonAsync("/api/v1/gestao-ordem-servico/ordens-servico/cadastrar", abertura, HttpStatusCode.Created);
        var ordemServicoId = ObterGuid(ordemServico, "id");

        // Act
        var statusRecebida = await GetJsonAsync($"/api/v1/gestao-ordem-servico/ordens-servico/{ordemServicoId}/consultar-status");
        ...
        var ordemComServicos = await PutJsonAsync($"/api/v1/gestao-ordem-servico/ordens-servico/{ordemServicoId}/definir-servicos", definicaoServicos);

        var statusComServico = await GetJsonAsync($"/api/v1/gestao-ordem-servico/ordens-servico/{ordemServicoId}/consultar-status");
        ...
        var ordemComPecasInsumos = await PostJsonAsync($"/api/v1/gestao-ordem-servico/ordens-servico/{ordemServicoId}/reservar-pecas-insumos", reservaPecas);
        ...
        var ordemFinalizada = await PostJsonAsync($"/api/v1/gestao-ordem-servico/ordens-servico/{ordemServicoId}/finalizar", request: null);

        var ordemEntregue = await PostJsonAsync($"/api/v1/gestao-ordem-servico/ordens-servico/{ordemServicoId}/entregar", request: null);
