...
public sealed class ListarOrdensServicoAbertasUseCase
{
    private readonly IOrdemServicoRepository _ordemServicoRepository;
    private readonly IValidator<ListarOrdensServicoRequest> _validator;
    private readonly IMapper _mapper;

    public ListarOrdensServicoAbertasUseCase(IOrdemServicoRepository ordemServicoRepository, IValidator<ListarOrdensServicoRequest> validator, IMapper mapper)
    {
    ...
    public async Task<Result<PagedResult<OrdemServicoResponse>>> ExecuteAsync(ListarOrdensServicoRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var ordensServico = await _ordemServicoRepository.ListarAbertasAsync(request.Pagina, request.TamanhoPagina, cancellationToken);
        var totalItens = await _ordemServicoRepository.ContarAbertasAsync(cancellationToken);
