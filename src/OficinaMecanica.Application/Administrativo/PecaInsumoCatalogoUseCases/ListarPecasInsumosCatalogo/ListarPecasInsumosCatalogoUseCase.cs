...
public sealed class ListarPecasInsumosCatalogoUseCase
{
    ...
    public ListarPecasInsumosCatalogoUseCase(IPecaInsumoCatalogoRepository pecaInsumoCatalogoRepository, IValidator<ListarPecasInsumosCatalogoRequest> validator, IMapper mapper)
    {
    ...
    public async Task<Result<PagedResult<PecaInsumoCatalogoResponse>>> ExecuteAsync(ListarPecasInsumosCatalogoRequest request, CancellationToken cancellationToken = default)
    {
        var validationResult = await _validator.ValidateAsync(request, cancellationToken);

        if (!validationResult.IsValid)
        {
            return Result<PagedResult<PecaInsumoCatalogoResponse>>.Falha(validationResult.ObterMensagensErro(), TipoErro.Validacao);
        }

        var itens = await _pecaInsumoCatalogoRepository.ListarAsync(request.Pagina, request.TamanhoPagina, cancellationToken);
        ...
        var pagedResult = new PagedResult<PecaInsumoCatalogoResponse>(response, request.Pagina, request.TamanhoPagina, totalItens);
