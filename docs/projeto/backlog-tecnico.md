...
| `F3-002` | `P0` | Fase 3 | API | Alinhar a API ao contrato único de JWT e manter a autorização dentro da aplicação. | API valida emissor, audiência, expiração, `jti`, papel e `cliente_id`; o perfil `Cliente` é removido do login interno somente quando o fluxo por documento cobrir CPF e CNPJ; perfis internos Admin, Atendente e Mecânico são preservados. | A fazer |
