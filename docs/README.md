# Documentação

Esta pasta reúne a documentação complementar do Tech Challenge Fase 2.

## Mapa

| Pasta | Conteúdo |
| --- | --- |
| `architecture/diagrams/c4-model/` | Modelo C4 oficial validado, com Structurizr DSL e SVGs exportados. |
| `architecture/diagrams/aws/` | Espaço reservado para os diagramas AWS finais. |
| `architecture/diagrams/deployment/kubernetes/` | Espaço reservado para os diagramas de deployment Kubernetes. |
| `architecture/diagrams/deployment/docker/` | Espaço reservado para os diagramas de deployment Docker. |
| `architecture/diagrams/ci-cd/` | Espaço reservado para os diagramas da esteira CI/CD. |
| `deploy/` | Guias de CI/CD, AWS, Terraform e destroy obrigatório. |
| `evidencias/` | Evidências versionadas de build, testes, cobertura, segurança, HPA e Terraform. |
| `openapi/` | Contrato OpenAPI exportado da API. |
| `projeto/` | Backlog, decisões e pendências da entrega. |

## Regras

- Não versionar credenciais, tokens, kubeconfig, secrets ou outputs sensíveis.
- Diagramas ainda não finalizados devem permanecer apenas como diretórios reservados com `.gitkeep`.
- Evidências com prints reais devem ser coladas antes da montagem do PDF final.
