...
> O enunciado da Fase 2 pede apenas que se "documente quais recursos estão sendo criados e como aplicar" — não especifica onde/como o Terraform deve guardar seu state. Esta é uma decisão técnica nossa. O Terraform precisa lembrar, entre uma execução e outra da esteira, quais recursos AWS já foram criados (o "state"). O padrão mais comum em produção é guardar esse state remotamente em um bucket S3 dedicado. No AWS Academy Learner Lab, porém, a política de conta (`voc-cancel-cred`) às vezes nega a permissão `s3:CreateBucket`, o que travaria a esteira sem nenhuma alternativa se o backend S3 fosse obrigatório. Esta ADR documenta, inclusive, uma mudança de rota: o projeto começou com backend S3 e migrou para backend local depois de esbarrar nessa restrição real do laboratório.
...
- **Enunciado da Fase 2** não exige backend remoto dedicado, apenas infraestrutura como código funcional e documentada.
...
- Exige um mecanismo de contingência para quando o state diverge da realidade (ver ADR-0011, workflow `AWS Import Existing Resources`).
